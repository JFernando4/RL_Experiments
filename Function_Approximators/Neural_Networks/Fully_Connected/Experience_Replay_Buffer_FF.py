...
class Buffer:
    def __init__(self, buffer_size, dimensions):
        ...
        observation_dimensions = n1 * n2 * d0
        output_dimensions = m2
        self.buffer_frames = np.zeros(shape=[buffer_size, observation_dimensions], dtype=float)
        self.buffer_actions = np.zeros(shape=[buffer_size, output_dimensions], dtype=int)
    ...
    def add_to_buffer(self, buffer_entry):
        if self.buffer_size > self.current_buffer_size:
            ...
            self.buffer_frames[self.current_buffer_size] = buffer_entry[1]
            ...
            self.buffer_frames[random_entry] = buffer_entry[1]
